...
author: "Steffi Reisenauer" # Remember to add your name!
date: "24-07-2024"
theme: united
---
...
cantril %>% 
  ...
  mutate(relative_change = round(absolute_change/`2011`, 2)) %>%
  mutate(relative_change = scales::percent(relative_change)) %>%
  rename(`Absolute change` = absolute_change,
...
p <- cantril %>% 
  filter(Country %in% c("Afghanistan",
                        ...
                        "Laos")) %>% 
  ggplot(aes(x = Year,
             y = `Cantril ladder score`, group = Country, color = Country))+
  geom_line()+
...
function filter_default() {
  var default_countries = ["Ukraine", "Albania", "Afgahnistan", "Somalia", "Laos", "Venezuela"]; // Set default countries here
